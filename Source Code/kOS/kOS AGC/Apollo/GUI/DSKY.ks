...
LOCAL FUNCTION DSKY_INDICATOR_NULLCHECK {
    ...
    IF _indicatorAtlas[ID] = "BLANK" and NOT(INDFIRSTRUN[ID]) { return true. }
    IF INDFIRSTRUN[ID] { set INDFIRSTRUN[ID] to false. }
    IF _indicatorAtlas[ID] = "NULL" { return true. }
    ELSE { return false. }
}
...
FUNCTION DSKY_JSON_OUTPUT {
    ...
    local iindx is 1.
    FOR i in _Ioutput {
        set kOSAGCCONFIG["I" + iindx:tostring] to i.
        set iindx to iindx+1.
    }
    WRITEJSON(_writelex, "0:/kOS AGC/DSKY/AGCoutput.json").
...
>>>>>>> b23e37abb8291893fb8d8e5d8cd113be9e959f5f
